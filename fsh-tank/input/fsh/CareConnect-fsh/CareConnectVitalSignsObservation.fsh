Profile: CareConnect-VitalSigns-Observation-1
Parent: Observation
Id: CareConnect-VitalSigns-Observation-1
...
* category ^binding.valueSet = "http://hl7.org/fhir/ValueSet/observation-category"
...
* code.coding[snomedCT].code = #118227000 (exactly)
* code.coding[snomedCT].display 1..
* code.coding[snomedCT].display = "Vital signs" (exactly)
...
//* context only Reference(CareConnect-EpisodeOfCare-1 or CareConnect-Encounter-1)* effective[x] 1.. MS
* effective[x] 1.. MS
* effective[x] ^short = "Clinically relevant time/time-period for theobservation"
...
* valueQuantity[valueQuantity] ^binding.description = "A value set describing common UCUM units for recording vital signs"
...
* method.coding contains snomedCT 0..*
...
// * related ^mustSupport = false
// * related.type 1.. MS
// * related.type = #has-member (exactly)
// * related.target only Reference(CareConnect-VitalSigns-Observation-1 or QuestionnaireResponse or Sequence)
// * related.target MS
* component MS
* component.code MS
...
* component.code.coding contains
    loinc 1..1 and
    snomedCT 1..1
* component.code.coding[loinc] ^binding.strength = #extensible
* component.code.coding[loinc] ^binding.description = "This value set indicates the allowed vital sign result types"
* component.code.coding[loinc] ^binding.valueSet = observation-vitalsignresult
...
* component.valueQuantity[valueQuantity] ^binding.description = "A value set describing common UCUM units for recording vital signs"
...
* component.valueQuantity[valueQuantity].system ^short = "System that defines the Unified Code for Units of Measure (UCUM)."
* component.valueQuantity[valueQuantity].code 1.. MS
* component.valueQuantity[valueQuantity].code ^short = "Coded responses from the common UCUM units for vital signs value set."
* component.valueQuantity[valueQuantity].code ^binding.strength = #required
* component.valueQuantity[valueQuantity].code ^binding.description = "A value set for common UCUM units for recording vital signs"
* component.valueQuantity[valueQuantity].code ^binding.valueSet = ucum-vitals-common
